...
class Dnd5eCharacter:
    ...
    def normalized_class_entries(self) -> list[dict[str, Any]]:
        from backend.games.dnd5e.characters.multiclass import normalize_class_entries

        return normalize_class_entries(self)

    def sync_legacy_class_fields(self) -> None:
        from backend.games.dnd5e.characters.multiclass import sync_legacy_class_fields

        sync_legacy_class_fields(self)
    ...
    def clamp(self) -> None:
        ...
        from backend.games.dnd5e.characters.creation_choices import sync_feature_choice_fields
        ...
        self._normalize_classes_field()
        self.sync_legacy_class_fields()

    def _normalize_classes_field(self) -> None:
        ...
        for item in self.classes:
            ...
            lv = max(0, min(20, int(item.get("level", 0) or 0)))
            if not cid or lv <= 0:
                continue
            cleaned.append(
                {
                    ...
                    "class_skill_choices": _clean_list(item.get("class_skill_choices"), 4),
                }
            )
...
def character_from_dict(data: dict[str, Any] | None) -> Dnd5eCharacter:
    ...
    char = Dnd5eCharacter(
        ...
        game_id=str(data.get("game_id", "dnd5e") or "dnd5e"),
...
def format_summary(char: Dnd5eCharacter) -> str:
    ...
    if char.class_name:
        ...
        if len(entries) > 1:
            parts.append(
                " / ".join(
                    f"{e['class_name'].title()} {e['level']}"
                    + (f" ({e['subclass']})" if e.get("subclass") else "")
                    for e in entries
                )
            )
        else:
...
def format_for_prompt(
    ...
    *,
    ...
    from backend.games.dnd5e.characters.character_builder import compute_spell_slots
    from backend.games.dnd5e.characters.creation_choices import (
        class_choice_lines,
        collect_derived_feats,
        species_trait_lines,
    )
    from backend.games.dnd5e.characters.features import features_summary_lines
    from backend.games.dnd5e.characters.origin_feats import origin_feat_passive_lines
    from backend.games.dnd5e.characters.spell_resources import compute_wild_shape_max
    ...
    for lvl in all_levels:
        ...
        if mx > 0:
            slot_parts.append(f"L{lvl}:{rem}/{mx}")
    slots = ", ".join(slot_parts) if slot_parts else "(none)"
    ws_max = compute_wild_shape_max(char)

    mc_parts = []
    for e in char.normalized_class_entries():
        part = f"{e['class_name']} {e['level']}"
        if e.get("subclass"):
            part += f" ({e['subclass']})"
